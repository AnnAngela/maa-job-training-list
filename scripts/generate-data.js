...
export const stripVolatileFields = (value) => {
...
export const isDataEquivalent = (before, after) => JSON.stringify(stripVolatileFields(before)) === JSON.stringify(stripVolatileFields(after));
...
export const parseSkillSpriteCss = (css) => {
    ...
    for (const classMatch of css.matchAll(classPattern)) {
        const blockStart = classMatch.index + classMatch[0].length;
