...
export const hmacSha256Hex = async (message, key, cryptoImpl = globalThis.crypto) => {
...
export const getSign = async (path, params = "", token, { cryptoImpl = globalThis.crypto, now = Date.now } = {}) => {
...
export const buildHeaders = async (path, params, cred, token, options) => {
...
export const fetchBindingList = async (fetchImpl, cred, token, options = {}) => {
...
export const chooseDefaultBinding = (bindingList) => {
...
export const fetchPlayerInfo = async (fetchImpl, uid, cred, token, options = {}) => {
...
export const formatSklandCharacters = (chars, operatorMeta, equipmentInfo = {}) => {
    const operators = [];
...
export const getSklandOperatorData = async (fetchImpl, cred, token, operatorMeta, { uid, ...options } = {}) => {
    let selectedUid = uid;
    let bindingList = [];
    if (!selectedUid) {
